...
- 很多 Skill 脚本需要重复编写相同的代码，缺乏统一、可复用的工具函数库
- 每次生成调用的 shell 代码可能不同，容易出错，特别在windows和linux终端执行命经常出错
- 提供更高效、更省ai token的方式 用于执行命令、读取大文件内容，比如读取大数据量json文件

### 项目目标
提供最简化的调用形式，支持命令行和代码导入两种方式，方便开发agent skill时直接调用，以及skill的脚本代码中导入使用，减少重复代码，提高开发效率，提升skill执行效率
